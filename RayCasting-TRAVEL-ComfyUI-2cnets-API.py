...
def main():
    ...
    size = 50    # size of the map
    posx, posy, posz = (1, 1, 0.5)
    rot, rot_v = (0, 0)
    lx, ly, lz = (size/2-0.5, size/2-0.5, 1)    
    mapc, maph, mapr = room_generator(posx, posy, size)
    ...
    screen = pg.display.set_mode((1024, 512),pg.RESIZABLE)
    ...
    while running:
        ...
        for i in range(nuc):
            lista.append([i, param_values[i*int(tam/nuc):(i+1)*int(tam/nuc)],
                          mapc, maph, lx, ly, lz, mapr, posx, posy, posz, mod])
        ...
        posx, posy, rot, rot_v, walkSpeed, turnFactor = keyboardMovement(pressed_keys, posx, posy, rot, rot_v, maph, clock.tick()/500, walkSpeed, turnFactor)       

        rot = rot + turnFactor
        posx, posy = (posx + walkSpeed*np.cos(rot), posy + walkSpeed*np.sin(rot))
        ...
        if renderButtons == True:
            for object in objects:
                ...
                if key == "go":                  
                    ...
                    #renderButtons = False
                    #renderFrame = True
                    walkSpeed = walkSpeed + 0.001
                    
                if key == "left":
                    turnFactor = turnFactor + 0.0001
                    #rot = rot + math.pi/4
                    #renderButtons = False
                    #renderFrame = True 
                if key == "right":
        ...
        if renderFrame == True:
            ...
            if blitDelay >= delayTrigg:
                blitDelay = 0
                renderFrame = False # disable render until new image was loaded
                ...
                thread = Thread( target=startWorkflow, args=(encoded_image,cnetImg) )   #, args=(encoded_image,encoded_sd_image)
                thread.start()
...
def startWorkflow( encodedImage,cnetImg ):
    ...
    # you can edit it while the script is running, if you like
    ...
    # override the text prompt for our positive CLIPTextEncode
    #prompt["6"]["inputs"]["text"] = "masterpiece best quality man"

    # override the seed for our KSampler node
    # prompt["3"]["inputs"]["seed"] = 704184315842712 #random.random() * 1000000000
    
    # you can send the image through the A1111/SDNext API as a binary blob
    # but this doesn't seem to work (yet), so I...
    # save current depth map as temporary file (after I created the folder inside the /input folder) 
    ...
    # the index may change if you edit the workflow.json! 
...
def room_generator(posx, posy, size):
    mapc = np.random.uniform(0, 0.1, (size,size,3)) 
    mapr = np.random.choice([0, 0, 0, 0], (size,size))
    maph = np.random.choice([0, 1], (size,size))
    maph[0,:], maph[size-1,:], maph[:,0], maph[:,size-1] = (1,1,1,1)

    mapc[posx][posy], maph[posx][posy], mapr[posx][posy] = (0, 0, 0)
...
def maze_generator(x, y, size):
    mapc = np.random.uniform(0, 0.1, (size,size,3)) 
    mapr = np.random.choice([0, 0, 0, 0], (size,size))
    maph = np.random.choice([0, 0, 0, 0, 0, 0, 0, 0, 1], (size,size))
    maph[0,:], maph[size-1,:], maph[:,0], maph[:,size-1] = (1,1,1,1)
...
def keyboardMovement(pressed_keys,posx, posy, rot, rot_v, maph, et, walkSpeed, turnFactor):
    
    x, y = (posx, posy)
    
    # p_mouse = pg.mouse.get_pos()
    # rot = rot + 4*np.pi*(0.5-(p_mouse[0]-400)/150000)
    # rot_v = np.cos(et*0.1) * 0.1 # vertical view angle
    ...
    return posx, posy, rot, rot_v, walkSpeed, turnFactor
...
def view_ray(x, y, z, cos, sin, sinz, mapc, lx, ly, lz, maph):
...
def caster(lista):
    ...
    mapc = lista[2]
    maph = lista[3]
    lx = lista[8] # put the light on player position
    ly = lista[9]
    lz = lista[10] - 1
    #exitx = lista[7]
    #exity = lista[8]
